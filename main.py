...
def CalcTrajectories():
    ...
    multi_traj.plot(color="purple")
    
    
    coords_array = [list(map(list, line.coords)) for line in multi_traj.geometry.geoms]

    return coords_array

if __name__ == "__main__":
    coords_array = CalcTrajectories()

    url = 'http://127.0.0.1:8080'
    headers = {'Content-Type': 'application/json'}

    json_data = json.dumps(coords_array)
    ...
    if len(sys.argv) > 1:
        ...
        response = requests.post(url, json=coords_array, headers=headers)
